...
export default function AppleBurnBadge({ className = "", tokenData }) {
  const burnedDisplay = tokenData?.totalBurnedFormatted || BURN_DATA.totalBurned || "24.8M";
  const percentDisplay = tokenData?.percentBurned || BURN_DATA.percentSupply || "2.48%";
  ...
  return (
    <a
      ...
      title={`Verified Onchain Burn: ${burnedDisplay} $MIKA (${percentDisplay}) on Robinhood Chain Blockscout`}
    >
      ...
      <span className="text-[9px] sm:text-[10px] font-mono font-bold tracking-[0.1em] uppercase text-white/90 group-hover/burn:text-white transition-colors flex items-center gap-1">
        <span>BURNED</span>
        <span className="text-[#d4fc50]">{burnedDisplay}</span>
        <span className="text-white/40 text-[9px] font-normal">({percentDisplay})</span>
      </span>
